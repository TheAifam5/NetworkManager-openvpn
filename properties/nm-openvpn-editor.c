...
static const char *compress_values[] = {
	"yes",
	"lz4",
	"lzo",
};
...
advanced_dialog_new (GHashTable *hash, const char *contype)
{
	...
	store = gtk_list_store_new (1, G_TYPE_STRING);
	active = 0;
	for (i = 0; i < G_N_ELEMENTS (compress_values); i++) {
		gtk_list_store_append (store, &iter);
		gtk_list_store_set (store, &iter,
		                    0, compress_values[i],
		                    -1);
		if (nm_streq (compress_values[i], value ?: "yes"))
			active = i;
	}
	gtk_combo_box_set_model (GTK_COMBO_BOX (combo), GTK_TREE_MODEL (store));
	g_object_unref (store);
...
advanced_dialog_new_hash_from_dialog (GtkWidget *dialog)
{
	...
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (widget))) {
		...
		if (active >= 0 && active < G_N_ELEMENTS (compress_values))
			g_hash_table_insert (hash, g_strdup (NM_OPENVPN_KEY_COMPRESS), g_strdup (compress_values[active]));
	}
